...
AddEventHandler("disc-inventoryhud:MoveToEmpty", function(data)
    ...
    if data.originOwner == data.destinationOwner and data.originTier.name == data.destinationTier.name then
        ...
        if data.destinationTier.name == 'drop' then
            if data.originItem.id == Config.BagItem then
                MySQL.Sync.execute("UPDATE `disc_inventory` SET `drop`=@drop WHERE owner = @owner AND type = @type", { 
...
ESX.RegisterServerCallback('disc-inventoryhud:GetLastAddedItemSlot', function(source, cb)
...
RegisterNetEvent('disc-inventory:saveInventory')
AddEventHandler('disc-inventory:saveInventory', function()
     saveInventoriesuni()
end)
